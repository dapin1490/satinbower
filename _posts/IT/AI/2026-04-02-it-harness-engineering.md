...
<script>
  document.addEventListener("DOMContentLoaded", function() {
    // #ref 또는 #fn으로 시작하는 모든 앵커 태그 선택
    const links = document.querySelectorAll('a[href^="#ref"], a[href^="#fn"]');

    links.forEach(link => {
      const targetId = link.getAttribute('href').slice(1);
      // id에 콜론(:)이 포함된 경우를 대비하여 CSS.escape 사용
      const targetElement = document.getElementById(targetId);

      if (targetElement) {
        link.classList.add('custom-tooltip');

        // 대상 요소 내의 텍스트 추출 (역링크 기호 ↩ 등은 제거)
        let tooltipText = targetElement.innerText.replace(/↩/g, '').trim();
        link.setAttribute('data-content', tooltipText);
      }
    });
  });
</script>
